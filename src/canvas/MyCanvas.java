...
public class MyCanvas extends JPanel implements MouseListener{
    ...
    Cell cell[];                  //定义改造的按钮数组3*3
    Rectangle cellNull;           //定义空方格区域
    public static int pictureID=1;//当前选择的图片代号
    ImageIcon imagels;
    public MyCanvas() {
        this.setLayout(null);
        this.setSize(400,400);//设置面板宽高300*300
        cellNull=new Rectangle(300,300,100,100);//空白方格区域在第三行第三列
        cell=new Cell[16];
        
        for (int i = 0; i < 4; i++) {//为9个方格加载图片，并初使化坐标，形成三行三列
            for(int j=0;j<4;j++){
            	imagels=new ImageIcon("images//pt2_"+(i*4+j+1)+".gif");  //实例化图标            	
                imagels.setImage(imagels.getImage().getScaledInstance(100,100,Image.SCALE_DEFAULT));                
                cell[i*4+j]=new Cell(imagels);
    ...
    private boolean test(int x,int y){//判断出界
